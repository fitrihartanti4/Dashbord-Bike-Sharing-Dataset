...
tahun = st.sidebar.selectbox("Pilih Tahun", options=tahun_options, format_func=lambda x: "Semua" if x == "All" else ("2011" if x == 0 else "2012"))
...
if tahun == "All":
    ...
    day_filtered = day_df[(day_df['yr'] == tahun) & (day_df['season'].isin(musim))]
    hour_filtered = hour_df[hour_df['yr'] == tahun]
...
# Histogram Penyewaan Sepeda Berdasarkan Hari
st.subheader("Distribusi Penyewaan Sepeda Berdasarkan Hari dalam Seminggu")
fig = px.histogram(day_filtered, x="weekday", nbins=7, title="Penyewaan Sepeda Berdasarkan Hari dalam Seminggu",
                   labels={"weekday": "Hari dalam Seminggu", "cnt": "Jumlah Penyewaan"})
st.plotly_chart(fig)

# Box Plot Pengaruh Kecepatan Angin terhadap Penyewaan Sepeda
st.subheader("Pengaruh Kecepatan Angin terhadap Penyewaan Sepeda")
fig = px.box(day_filtered, x="windspeed", y="cnt", title="Box Plot Penyewaan Sepeda terhadap Kecepatan Angin")
st.plotly_chart(fig)
